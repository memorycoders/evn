...
function Email(props) {
    ...
    const fetchData = async (pageIndex) => {
        ...
        try {
            const rs = await http.get(`web/admin/templates?template_type_id=${TEMPLATES_TYPE_ID.EMAIL}&pageIndex=${pageIndex}&pageSize=5`);
            if (rs?.status === 200) {
    ...
    const onFinish = async (values) => {
        try {
            ...
            var request = {
                ...
                "templateTypeId": TEMPLATES_TYPE_ID.EMAIL
           ...
           NotificationError('', `Có lỗi xảy ra " ${error.message} " Vui lòng thử lại`);
        }
    }
